...
@mcp.tool()
async def search_exploits(query: str, max_results: int = 100) -> List[str]:
    """
    Simple search for explicit exploit modules using Metasploit's built-in search.

    This taps into MSF's own search (name, description, cve, author, rank, platform, etc.).
    The query is passed through to the console, e.g.:
      - "ms17_010"
      - "cve:2017 platform:windows port:445"
      - "wordpress type:exploit rank:excellent"

    Args:
        query: Free-form Metasploit search query. Recommended to include 'type:exploit'.
        max_results: Maximum number of module paths to return.

    Returns:
        List of exploit module paths (e.g., 'exploit/windows/smb/ms17_010_eternalblue').
        On error, returns a single-item list with an error string starting with 'Error: ' (consistent with list_exploits).
    """
    if not isinstance(query, str) or not query.strip():
        return ["Error: query must be a non-empty string."]

    search_cmd = f"search {query}" if "type:" not in query else f"search {query}"
    logger.info(f"Executing advanced exploit search: {search_cmd}")

    try:
        async with get_msf_console() as console:
            output = await run_command_safely(console, search_cmd, execution_timeout=LONG_CONSOLE_READ_TIMEOUT)

        # Primary extraction: grab explicit module paths like exploit/... from the output
        module_paths: List[str] = []
        seen = set()
        for m in re.finditer(r"(?im)(exploit/[a-z0-9_\-]+(?:/[a-z0-9_\-]+)+)", output):
            mod = m.group(1).strip()
            if mod not in seen:
                seen.add(mod)
                module_paths.append(mod)
                if len(module_paths) >= max_results:
                    break

        # Fallback: try to parse table rows if regex found nothing
        if not module_paths:
            for line in output.splitlines():
                if '|' in line and 'exploit/' in line:
                    # Attempt to extract module path between pipes or the first token containing 'exploit/'
                    candidates = re.findall(r"exploit/[a-z0-9_\-]+(?:/[a-z0-9_\-]+)+", line, flags=re.IGNORECASE)
        ...
        logger.info(f"Advanced search found {len(module_paths)} exploit modules for query '{query}'.")
        return module_paths
    except Exception as e:
        logger.exception("Unexpected error during advanced search")
        return [f"Error: Unexpected error during advanced search: {e}"]
...
async def search_advanced(
    ...
    """
    Advanced module search, with ranking, explicit filters, and optional partial-code search. 

    Args:
        query: Free-form search terms (e.g., "ms17_010", "wordpress upload").
        filters: Dict or string ("key=value,key2=value2"). Supported keys include
                 rank, platform, arch, cve, author, port, check, name. Values are passed
                 through to MSF's search as "key:value" (except name -> plain term).
        code_contains: Optional substring to search for inside module source code.
        max_results: Max results to return (after ranking/filtering).
        scan_limit: Max number of candidates to examine for code_contains filtering.
        module_type: Optional module type filter. One of:
                     exploit, auxiliary, post, payload, encoder, nop. If None, searches all.

    Returns:
        List of objects: { module, score, source_path? }
    """
    # Parse filters gracefully
    try:
        parsed_filters = _parse_options_gracefully(filters)
    except ValueError as e:
        return [{"error": f"Invalid filters: {e}"}]

    # Build MSF search command
    allowed_types = {"exploit", "auxiliary", "post", "payload", "encoder", "nop"}
    if module_type is not None:
        mt_norm = str(module_type).strip().lower()
        if mt_norm not in allowed_types:
            return [{"error": f"Invalid module_type '{module_type}'. Allowed: {sorted(allowed_types)}"}]
    else:
        mt_norm = None
    ...
    for key, value in (parsed_filters or {}).items():
        ...
        if k == 'name':
            ...
            # Escape spaces by quoting if needed
    ...
    msf_query = base_terms
    if mt_norm:
        msf_query = (f"type:{mt_norm} " + msf_query).strip()
    if filter_parts:
        msf_query += " " + " ".join(filter_parts)

    logger.info(f"Executing search_exploits_advanced with query: {msf_query}")

    # Step 1: Run MSF search and parse module paths
    try:
        async with get_msf_console() as console:
            output = await run_command_safely(console, f"search {msf_query}", execution_timeout=LONG_CONSOLE_READ_TIMEOUT)
    except Exception as e:
        logger.exception("Error running MSF search in advanced search")
        return [{"error": f"Search failed: {e}"}]

    candidates: List[str] = []
    seen = set()
    # Capture modules of any supported type
    for m in re.finditer(r"(?im)((?:exploit|auxiliary|post|payload|encoder|nop)/[a-z0-9_\-]+(?:/[a-z0-9_\-]+)+)", output):
    ...
    if not candidates:
        for line in output.splitlines():
            if any(t in line for t in ['exploit/', 'auxiliary/', 'post/', 'payload/', 'encoder/', 'nop/']):
                for mod in re.findall(r"(?:exploit|auxiliary|post|payload|encoder|nop)/[a-z0-9_\-]+(?:/[a-z0-9_\-]+)+", line, flags=re.IGNORECASE):
                    if mod not in seen:
                        seen.add(mod)
    ...
    query_tokens = tokenize(base_terms)
    # Include filters as tokens for scoring
    for k, v in (parsed_filters or {}).items():
        if k == 'name':
            query_tokens.extend(tokenize(str(v)))
        else:
            query_tokens.extend(tokenize(str(v)))

    def score_module(mod: str) -> int:
        score = 0
        parts = mod.lower().split('/')
        name_tokens = tokenize(parts[-1])
        path_tokens = set(tokenize(mod))
        # Name token overlap gets higher weight
        for qt in query_tokens:
            if qt in name_tokens:
                score += 15
            elif qt in path_tokens:
                score += 7
        # Heuristic boosts for common platforms present in path
        plat = str(parsed_filters.get('platform', '')).lower() if parsed_filters else ''
        if plat and plat in path_tokens:
            score += 10
        arch = str(parsed_filters.get('arch', '')).lower() if parsed_filters else ''
        if arch and arch in path_tokens:
            score += 6
        cve = str(parsed_filters.get('cve', '')).lower() if parsed_filters else ''
        if cve and cve in mod.lower():
            score += 12
        # Prefer well-known/targeted services if present (quick heuristic)
        for svc in ['smb', 'rdp', 'http', 'ftp', 'ssh', 'mysql', 'mssql', 'oracle', 'vnc']:
            if svc in path_tokens and any(svc in qt for qt in query_tokens):
                score += 5
        return score

    scored = [{"module": m, "score": score_module(m)} for m in candidates]
    scored.sort(key=lambda x: x["score"], reverse=True)

    # Step 3 (optional): partial-code search; filter and boost by occurrences
    if code_contains:
        term = code_contains
        term_l = term.lower()
        ...
        async with get_msf_console() as console:
            for item in scored:
                ...
                try:
                    info_out = await run_command_safely(console, f"info {module_path}", execution_timeout=DEFAULT_CONSOLE_READ_TIMEOUT)
                    path_match = re.search(r"(?im)^\s*Path:\s*(.+)$", info_out)
                    if not path_match:
                        continue
                    ...
                    with open(source_path, 'r', encoding='utf-8', errors='replace') as f:
                        content = f.read()
                    if term_l in content.lower():
                        # Boost score by occurrence count (capped)
                        occ = min(content.lower().count(term_l), 10)
                        item['score'] += 3 * occ
                        item['source_path'] = source_path
                        filtered.append(item)
                except Exception:
                    # Skip modules that fail to inspect
                    pass
                finally:
                    checked += 1
        scored = filtered
...
@mcp.tool()
async def get_exploit_source(module_name: str, return_base64: bool = False) -> Dict[str, Any]:
    """
    Return the Ruby source code for a specific exploit module. 
    This can be helpful for understanding the module, for debugging or as an inspiration to write an own exploid.

    Args:
        module_name: Name/path of the exploit module. Accepts forms like
                     'windows/smb/ms08_067_netapi' or 'exploit/windows/smb/ms08_067_netapi'.
        return_base64: If True, returns base64-encoded source under 'code_base64' instead of 'code'.

    Returns:
        Dict with status, module canonical name, source_path, and code (or code_base64).
    """
    if not module_name or not isinstance(module_name, str):
        return {"status": "error", "message": "module_name must be a non-empty string."}

    # Normalize to a full module path for the console 'info' command
    # Detect module type prefix if present, otherwise default to 'exploit'
    module_type_prefixes = ['exploit', 'payload', 'post', 'auxiliary', 'encoder', 'nop']
    if '/' not in module_name:
        full_module_path = f"exploit/{module_name}"
    else:
        if not any(module_name.startswith(pfx + '/') for pfx in module_type_prefixes):
            full_module_path = f"exploit/{module_name}"
        else:
            full_module_path = module_name

    # Build the source file path directly from a fixed modules root
    modules_root = "/opt/metasploit-framework/embedded/framework/modules"
    type_dir_map = {
        'exploit': 'exploits',
        'auxiliary': 'auxiliary',
        'post': 'post',
    ...
    try:
        # Split module type and remainder
        if '/' in full_module_path:
            mt, remainder = full_module_path.split('/', 1)
        else:
            mt, remainder = 'exploit', full_module_path

        subdir = type_dir_map.get(mt.lower(), mt.lower())
        source_path = os.path.join(modules_root, subdir, remainder + '.rb')

        if not os.path.exists(source_path):
            logger.error(f"Module source path does not exist: {source_path}")
            return {"status": "error", "message": f"Module source file not found: {source_path}", "source_path": source_path, "module": full_module_path}

        with open(source_path, 'r', encoding='utf-8', errors='replace') as f:
            code_text = f.read()

        if return_base64:
            encoded = base64.b64encode(code_text.encode('utf-8', errors='replace')).decode('ascii')
            return {
                "status": "success",
                "module": full_module_path,
        ...
        logger.exception(f"Unexpected error obtaining source for {full_module_path}")
        return {"status": "error", "message": f"Unexpected error while retrieving source code: {e}", "module": full_module_path}
